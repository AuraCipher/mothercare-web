    ...
    <div className="rounded-xl border border-warm-card-border bg-warm-card p-4 group">
      ...
      {progress < 100 && (
        <div className="mt-3 h-1.5 rounded-full bg-warm-card-border/30 overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-500 ${progress > 80 ? 'bg-warm-accent' : 'bg-warm-accent/80'}`}
...
export default function DocNav() {
  const { entity, canCreate, canUpdate, canDelete, showDocNav } = useDocumentPermissions();
      ...
      .then((result) => {
        if (result.success && Array.isArray(result.data)) {
          setUploads((prev) => {
            ...
            return result.data.map((f: { id: string; originalName: string; mimeType: string; size: number }) => ({
              id: f.id,
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    setUploads((prev) => [
      ...prev,
      {
        id: tempId,
        name: file.name,
        type: `${fileType} · ${file.name.split('.').pop()?.toUpperCase() || 'FILE'}`,
        size: file.size,
        progress: 0,
        done: false,
      },
  ...
  const handleRename = useCallback(async (fileId: string, newName: string) => {
    if (!canUpdate) {
      showToast('error', 'No permission to rename documents');
  ...
  const handleDelete = useCallback(async (fileId: string) => {
    if (!canDelete) {
      showToast('error', 'No permission to delete documents');
      return;
    }
    const token = localStorage.getItem('token');
    if (!token) return;
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-300 ease-out ${drawerOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
        <div
          className={`absolute inset-0 bg-black/60 backdrop-blur-[2px] transition-opacity duration-300 ease-out ${drawerOpen ? 'opacity-100' : 'opacity-0'}`}
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            {canCreate ? (
              ...
              <p className="rounded-lg border border-warm-card-border bg-warm-card/30 px-3 py-2 text-xs text-warm-muted">
                Upload disabled — no document create permission.
              </p>
            )}
            ...
            {activeUploads.length > 0 && (
              <>
                ...
                {activeUploads.map((u) => (
                  <DocCard
                    key={u.id}
                    icon={u.progress > 0 ? (u.name.match(/\.(png|jpg|jpeg|webp)$/i) ? '🖼️' : '📄') : '📄'}
                    name={u.name}
                    type={u.type}
                    progress={u.progress}
                  />
            ...
            {completedUploads.length > 0 && (
              <>
                ...
                {completedUploads.map((u) => (
                  <DocCard
                    key={u.id}
                    icon={u.name.match(/\.(png|jpg|jpeg|webp)$/i) ? '🖼️' : '📄'}
                    name={u.name}
                    type={u.type}
                    progress={100}
                    showCheck
                    fileId={u.fileId}
                    allowRename={canUpdate}
            ...
            {uploads.length === 0 && (
              <p className="text-xs text-warm-muted/50 text-center pt-4">No documents yet.{canCreate ? ' Tap above to upload.' : ''}</p>
            )}
          </div>
        </div>
      </div>
