...
import { Building2, Calendar, Plus, ArrowLeft, CheckCircle, Archive, ExternalLink, ChevronDown, ChevronRight, Trash2 } from 'lucide-react';
...
export default function BranchDetailPage() {
  ...
  const handlePublish = async (ayId: string) => {
    if (!confirm('Publish this academic year? It will become ACTIVE. Only one AY can be active per branch.')) return;
    try {
  ...
  const handleArchive = async (ayId: string) => {
    if (!confirm('Archive this academic year? This will close it.')) return;
    try {
  ...
  const handleDeleteAy = async (ayId: string) => {
    if (!confirm('Delete this academic year? This can only be done for ARCHIVED years.')) return;
    try {
      await api.deleteAcademicYear(ayId);
      loadData();
    } catch (e: any) {
  ...
  const statusBadge = (status: string) => {
    const cfg = statusConfig[status as keyof typeof statusConfig] || statusConfig.ARCHIVED;
    return (
  ...
  if (!branch && !loading) {
    return (
      <main className="mx-auto max-w-5xl px-6 py-10">
        ...
        <button onClick={() => router.push('/admin/branches')} className="mt-3 text-xs text-warm-accent hover:text-[#b39a76]">← Back to branches</button>
      </main>
    );
  }
      ...
      {academicYears.length === 0 ? (
        ...
        <div className="space-y-2">
          {academicYears.map((ay) => (
            <div key={ay.id} className="rounded-xl border border-warm-card-border bg-warm-card overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="ml-2">{statusBadge(ay.status)}</span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-[10px] text-warm-muted/60">
                    {ay._count.groups} groups · {ay._count.students} students · {ay._count.members} members
                  </span>
                  <div className="flex items-center gap-1">
                    {ay.status === 'BUILD_STAGE' && (
                      <button
                        onClick={() => handlePublish(ay.id)}
              ...
              {expandedAy === ay.id && (
                <div className="border-t border-warm-card-border px-4 py-3 pl-12">
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    <div>
                      <p className="text-warm-muted/60">Groups</p>
                      <p className="text-warm-cream">{ay._count.groups}</p>
                    </div>
                    <div>
                      <p className="text-warm-muted/60">Students</p>
                      <p className="text-warm-cream">{ay._count.students}</p>
                    </div>
                    <div>
                      <p className="text-warm-muted/60">Members</p>
                      <p className="text-warm-cream">{ay._count.members}</p>
                    </div>
                  </div>
                </div>
              )}
      ...
      {showCreateAy && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setShowCreateAy(false)}>
          <div className="w-full max-w-md rounded-xl border border-warm-card-border bg-[#24201e] p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h2 className="mb-4 text-sm font-medium text-warm-cream">New Academic Year</h2>

            {calendars.length === 0 ? (
              <p className="text-xs text-warm-muted">No calendars available. Create a calendar first.</p>
            ) : (
              <div className="space-y-3">
                <label className="mb-1 block text-xs text-warm-muted">Select Calendar</label>
                {calendars.map((cal) => (
                  <label
                    key={cal.id}
                  ...
                  >
                    <input type="radio" name="calendar" checked={selectedCalendarId === cal.id} onChange={() => setSelectedCalendarId(cal.id)} className="h-3.5 w-3.5 text-warm-accent" />
                    <div>
                      <span className="text-sm text-warm-cream">{cal.label}</span>
                      {cal.isCurrent && (
                        <span className="ml-2 text-[10px] text-warm-accent">(Current)</span>
                      )}
            ...
            {createAyError && <p className="mt-3 text-xs text-[#b39a76]">{createAyError}</p>}
