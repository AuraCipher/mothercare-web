...
function ReportCardPreview({ bundle, card }: { bundle: ReportCardBundle; card: ExamReportCard }) {
  return (
    <div
      className="overflow-hidden rounded-lg border border-warm-card-border/60 bg-white text-[#1c1917] shadow-lg"
...
export default function ReportCardsPage() {
  ...
  const [schoolName, setSchoolName] = useState('Mother Care School');
  ...
  const generateCards = async () => {
    ...
    setBundle(null);
    try {
      const cards = await buildExamReportCards(sessionId, examId, classId);
      if (cards.length === 0) {
        showToast('error', 'No students or marks found for this class and exam');
        setLoading(false);
        return;
      }
    ...
    <main className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-warm-muted">
              Generate printable exam report cards per class
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-warm-card-border bg-warm-card p-5 mb-6">
        <p className="mb-4 text-[10px] uppercase tracking-wide text-warm-muted/50">
          Select session, exam &amp; class — all required
        </p>
          ...
          <div className="rounded-xl border border-warm-card-border bg-warm-card/70 px-5 py-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-[10px] text-warm-muted/50 mt-0.5">
                {bundle.generatedAt} · {bundle.cards.length} students · {bundle.sessionName}
              </p>
            </div>
