      ...
      {report && (
        <div className="rounded-xl border border-warm-card-border overflow-hidden">
          ...
          <div className="px-5 py-3 border-t border-warm-card-border/30 flex flex-wrap gap-4 text-xs text-warm-muted/70">
            {report.statusFilter ? (
              <span><span className="font-medium" style={{color: report.statusFilter === 'absent' ? '#ef4444' : report.statusFilter === 'present' ? '#22c55e' : report.statusFilter === 'late' ? '#eab308' : '#3b82f6'}}>{report.summary.statusCount || 0}</span> {STATUS_LABELS[report.statusFilter] || report.statusFilter} · {(report.summary.statusPercent || 0)}% of all · {report.total || 0} listed {reportTarget === 'teacher' ? 'teachers' : 'students'}</span>
            ) : report.isSingleDay ? (
              <span><span className="text-green-400 font-medium">{report.summary.present || 0}</span> Present · <span className="text-red-400 font-medium">{report.summary.absent || 0}</span> Absent · <span className="text-yellow-400 font-medium">{report.summary.late || 0}</span> Late · <span className="text-blue-400 font-medium">{report.summary.leave || 0}</span> Leave · {report.total} {reportTarget === 'teacher' ? 'teachers' : 'students'}</span>
            ) : (
              <><span><span className="text-green-400 font-medium">{report.summary.present || 0}</span> P</span><span><span className="text-red-400 font-medium">{report.summary.absent || 0}</span> A</span><span><span className="text-yellow-400 font-medium">{report.summary.late || 0}</span> L</span><span><span className="text-blue-400 font-medium">{report.summary.leave || 0}</span> Lv</span><span className="font-medium">{report.summary.percentage || 0}%</span></>
            )}
          </div>
