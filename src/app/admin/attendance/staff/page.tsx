...
export default function StaffAttendancePage() {
  ...
  const loadUrl = `${config.apiUrl}/admin/attendance/staff${scopeQuery(
    viewMode === 'day'
      ? { date }
  ...
  const filteredTeachers = useMemo(() => {
    ...
    return result;
  }, [staffList, searchQuery, statusFilter, viewMode]);

  const isSunday = (dateStr: string) => new Date(dateStr + 'T00:00:00').getDay() === 0;

  const getCellStatus = (staffUserId: string, dateStr: string) => {
    const attStatus = statusMap[staffUserId]?.[dateStr];
    if (attStatus) return attStatus;
    if (isSunday(dateStr)) return 'holiday';
    return 'unmarked';
  };

  const toggleStatus = (staffUserId: string) => {
    setStaffList((prev: any[]) => prev.map((t: any) => {
      if (t.id !== staffUserId) return t;
  ...
  const setNote = (staffUserId: string, note: string) => {
    setStaffList((prev: any[]) => prev.map((t: any) => {
      if (t.id !== staffUserId) return t;
  ...
  const markAll = (status: string) => {
    setStaffList((prev: any[]) => prev.map((t: any) => ({ ...t, attendances: [{ status }] })));
  };

  const markHoliday = async (dateStr: string) => {
    if (!token) return;
    setSaving(true);
    const records = staffList.map((t: any) => ({ staffUserId: t.id, status: 'holiday' }));
    try {
      const res = await fetch(`${config.apiUrl}/admin/attendance/staff/batch`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify(scopeBody({ date: dateStr, records })),
      });
      const json = await res.json();
      if (json.success) { showToast('success', `Holiday set for ${json.data.saved} staffList`); loadAttendance(); }
  ...
  const handleSave = async () => {
    ...
    const records = staffList
      ...
      .map((t: any) => ({ staffUserId: t.id, status: t.attendances[0].status, note: t.attendances[0]?.note || '' }));
    try {
      const res = await fetch(`${config.apiUrl}/admin/attendance/staff/batch`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <div className="mb-6 flex items-center gap-3">
        ...
        <h1 className="text-xl font-light text-warm-cream">Staff Attendance</h1>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="rounded-xl border border-warm-card-border  relative">
              ...
              <thead>
                {viewMode === 'year' && viewMonths ? (
                  <tr>
                    ...
                    {viewMonths.map((m) => (
                      <th key={m.label} className="px-1 py-3 text-xs text-warm-muted font-medium text-center bg-[#24201e] w-[52px] min-w-[52px]">
                    ...
                    return (
                      <tr key={t.id} className="border-t border-warm-card-border/30 hover:bg-warm-card/20 transition-colors">
                        ...
                        {viewMonths.map(m => {
                          ...
                          return (
                            <td key={m.label} className="px-1 py-2 text-center text-[11px] font-mono w-[52px] min-w-[52px]">
            ...
            {staffList.length === 0 && (
              <div className="p-12 text-center text-sm text-warm-muted">No staffList found.</div>
            )}
          </div>
        </>
      )}
