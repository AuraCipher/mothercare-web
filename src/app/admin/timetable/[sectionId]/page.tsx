...
export default function SectionTimetablePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.getTimetableSlots(branchId, ayId),
  ...
  for (const slot of slots) {
    if (!groupedSlots[slot.dayOfWeek]) groupedSlots[slot.dayOfWeek] = [];
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-warm-muted mt-0.5">Weekly schedule</p>
      </div>
      ...
      {slots.length === 0 ? (
        <div className="rounded-xl border border-warm-card-border bg-warm-card p-10 text-center">
          ...
          <p className="text-sm text-warm-muted">No lectures defined yet.</p>
          <p className="mt-1 text-xs text-warm-muted/60">Go to Timetable and add lectures first.</p>
        </div>
      ) : (
